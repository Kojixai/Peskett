...
export default async function OrdersPage() {
  ...
  const totalRevenue = orders.reduce((s, o) => s + (o.sale_price ?? 0), 0)
    ...
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)] mt-0.5">{orders.length} total sales</p>
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm">
        {orders.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-xs text-[var(--text-subtle)] mt-1">Connect Vinted Pro in Settings to sync your orders</p>
          </div>
        ) : (
          <>
                ...
                return (
                  <div key={order.id} className="p-4 space-y-2">
                    ...
                    {order.shipment_label_url && (
                      <a href={order.shipment_label_url} target="_blank" rel="noopener noreferrer"
                  ...
                  {orders.map((order: any) => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-[var(--bg-elevated)] transition-colors">
                        ...
                        <td className="px-4 py-3 font-mono text-xs text-[var(--text-muted)]">{formatCurrency(order.platform_fee + order.shipping_cost)}</td>
                        ...
                        <td className="px-4 py-3 font-mono text-xs text-[var(--text-muted)]">{order.margin_percent?.toFixed(1)}%</td>
